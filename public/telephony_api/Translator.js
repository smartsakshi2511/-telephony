function Translator() {
    var recognition;
    var isTranscribing = false; // Flag to track whether transcription is active

    // Existing voice-to-text function
    ...
    // Stop transcription (stop the SpeechRecognition)
    this.stop = function() {
        if (recognition) {
            recognition.stop();  // Stops the speech recognition
            console.log('Speech recognition stopped.');
            isTranscribing = false;  // Set the flag to false
        }
    };
    ...
    function initTranscript(callback, language) {
        if (recognition) recognition.stop(); // Stop any existing recognition before starting a new one
        ...
        recognition.onend = function() {
            if (isTranscribing) {
                // Only restart if transcription is still active
                initTranscript(callback, language); 
            }
        };
        ...
        recognition.start();
        isTranscribing = true; // Set the flag to true when transcription starts
    }
